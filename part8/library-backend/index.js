...
const resolvers = {
  Query: {
    ...
    allBooks: (_root, { author, genre }) => {
      return books
        .filter(b => author === undefined || b.author === author)
        .filter(b => genre === undefined || b.genres.includes(genre))
    },
    allAuthors: () => authors.map(a => {
      return {
        ...a,
        bookCount: books.reduce((acc, b) => b.author === a.name ? acc + 1 : acc, 0)
      }
    })
  },
  Mutation: {
    addBook: (_root, args) => {
      ...
      const author = authors.find(a => a.name === addedBook.author)
      if (!author) {
